  ...
  return (
    <div className="flex items-center gap-3 min-w-[220px] max-w-[300px] py-1">
      <div className="w-10 h-10 rounded-full bg-gray-300 dark:bg-gray-600 flex-shrink-0 flex items-center justify-center overflow-hidden">
        <svg width="20" height="20" viewBox="0 0 24 24" fill={isMe ? "#fff" : "#888"}><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3zm5-3c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.81 6.43 6.32 6.92V21h1.36v-3.08C16.19 17.43 19 14.53 19 11h-2z" /></svg>
      </div>
      ...
      <div className="flex flex-col flex-grow">
        <input type="range" min="0" max="100" value={progress || 0} onChange={handleSeek} className="w-full h-1.5 bg-gray-300 rounded-lg appearance-none cursor-pointer" style={{ accentColor: isMe ? '#00A884' : '#53bdeb' }} />
      ...
      <audio ref={audioRef} src={src} onTimeUpdate={handleTimeUpdate} onLoadedMetadata={(e) => setDuration(e.currentTarget.duration)} onEnded={() => setIsPlaying(false)} className="hidden" />
    </div>
  );
};

// ==========================================
// 2. PEMUTAR VIDEO (KUSTOM)
// ==========================================
const WhatsAppVideoPlayer = ({ src }: { src: string }) => {
  ...
  return (
    <div className="relative mt-1 rounded-md overflow-hidden bg-black max-w-sm group cursor-pointer" onClick={togglePlay}>
      <video ref={videoRef} src={src} preload="metadata" onEnded={() => setIsPlaying(false)} controls={isPlaying} className="w-full h-auto max-h-64 object-contain" />
      {!isPlaying && (
        <div className="absolute inset-0 bg-black/30 flex items-center justify-center transition-all group-hover:bg-black/40">
          <div className="w-14 h-14 bg-black/60 rounded-full flex items-center justify-center backdrop-blur-sm border border-white/20">
            <svg width="30" height="30" viewBox="0 0 24 24" fill="white" className="ml-1"><path d="M8 5v14l11-7z" /></svg>
          </div>
        </div>
      )}
    </div>
...
export function MessageBubble({ message, isMe, showSender, attachmentMap, onImageClick, searchQuery = '' }: MessageBubbleProps) {

  // Fungsi Stabilo Kuning/Hijau
  const renderHighlightedText = (text: string) => {
    if (!searchQuery.trim()) return text;

    // Mencegah crash jika pengguna mengetik simbol Regex seperti tanda tanya (?)
    ...
    return parts.map((part, i) =>
      regex.test(part) ? (
        <mark key={i} className="bg-[#00a884] text-white px-0.5 rounded-sm shadow-sm">{part}</mark>
      ) : (
        part
      )
    );
    ...
    if (matches.length > 0) {
      ...
      return (
        <div className="flex flex-col gap-1 pr-12">
          ...
          {videos.map((vid, idx) => <WhatsAppVideoPlayer key={idx} src={vid.url} />)}
          {audios.map((aud, idx) => <VoiceNotePlayer key={idx} src={aud.url} isMe={isMe} />)}
          ...
          {caption && <span className="text-[14.2px] leading-relaxed whitespace-pre-wrap break-words mt-1">{renderHighlightedText(caption)}</span>}
        </div>
      );
    }

    // Jika pesan murni teks, aplikasikan fungsi Stabilo
    return <div className="text-[14.2px] leading-relaxed whitespace-pre-wrap break-words pr-12">{renderHighlightedText(message.content)}</div>;
  };
